...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        await m.reply_text(f" ** ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉**\n@{UPDATES_CHANNEL}\n» **اشترك بقناة البوت لتستطيع تشغيل الاغاني**")
    else:
        ...
        if not a.can_invite_users:
            await m.reply_text("اعطي البوت صلاحية:" + "\n\n» ❌__إضافة مستخدمين__")
            return
        try:
        ...
        if replied:
            if replied.audio or replied.voice:
                suhu = await replied.reply("**يتم البحث عن الاغنيه 🔎.**")
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await m.reply_photo(
                            photo=f"{IMG_2}",
                            caption=f"-› الحاله : تم التشغيل بنجاح\n🏷 -› الاسم: [{songname}]({link})\n💭-› ايدي المحادثه: {chat_id}`\n💡 ** الحالة:**ل بنجاح`\n🎧 **-› طلب الحلو:** {requester}\n📹 ** نوع البث:** `موسيقى",
                            reply_markup=keyboard,
                        )
                    except Exception as e:
                        await suhu.delete()
                        await m.reply_text(f"🚫 حدث خطأ تاكد من المكالمة مفتوحه  اولا:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "»قم بالرد ع  ملف صوتي  او  اكتب شيء للبحث**"
                    )
                else:
                    ...
                    if search == 0:
                        await suhu.edit("❌ **لم يتم أّلعثور على نتأئج.**")
                    else:
                        songname = search[0]
                        url = search[1]
                        duration = search[2]
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                await m.reply_photo(
                                    photo=thumbnail,
                                    caption=f"💡 ***-› تم ضفتها للانتظار** `{pos}`\n\n🏷 **-› الاسم:** [{songname}]({url})| موسيقى`\n**⏱ المدة:** `{duration}`\n🎧 **-› طلب الحلو:** {requester}",
                                    reply_markup=keyboard,
                                )
                            else:
                                try:
                                    await suhu.edit("🔄 ** تم الانضمام للمكالمه والتشغيل...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 حدث خطأ تاكد من المكالمة مفتوحه  اولا: `{ep}`")

        else:
            if len(m.command) < 2:
                await m.reply(
                    "» الرد علي ** ملف صوتي  ** أو ** اعطي شيئا للبحث.**"
                )
            else:
                ...
                search = ytsearch(query)
                if search == 0:
                    await suhu.edit("❌ **لم يتم العثور علي نتأئج.**")
                else:
                    songname = search[0]
                    url = search[1]
                    duration = search[2]
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                await suhu.edit("🔄 **الانضمام الي المكالمة...**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioPiped(
                                        ytlink,
                                ...
                                await suhu.delete()
                                await m.reply_text(f"🚫حدث خـطا تئكيد من المكالمة مفتوحه  اولا`{ep}`")
